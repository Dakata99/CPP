...
bool GList<Vertex, Edge>::has_edge(const Vertex& v1, const Vertex& v2) const
{
    ...
    if(this->oriented)
    {
        for(const my_pair& p : l) if(p.first == v2) return true;
...
{   
    ...
    for(auto it = set.begin(); it != set.end(); it++)
    {
        for(const my_pair& pair : it->second)
...
template<class Vertex, class Edge>
void GList<Vertex, Edge>::print_degrees() const
{
    for(auto it = set.begin(); it != set.end(); it++)
    {
...
template<class Vertex, class Edge>
size_t GList<Vertex, Edge>::count_of_vertices() const { return set.size(); }

template<class Vertex, class Edge>
size_t GList<Vertex, Edge>::count_of_edges() const
{
    size_t edges = 0;
