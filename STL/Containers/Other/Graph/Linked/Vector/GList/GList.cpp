...
void GList<Vertex, Edge>::add_oriented_weighted_edge(const Vertex& v1, const Vertex& v2, const Edge& edge_value)
{
    ...
    for (size_t i = 0; i < vertices.size(); i++)
        ...
        {    vertices[i].second.push_back( {v2, edge_value} ); break;   }
}
...
void GList<Vertex, Edge>::add_oriented_non_weighted_edge(const Vertex& v1, const Vertex& v2)
{
    ...
    for (size_t i = 0; i < vertices.size(); i++)
        if(vertices[i].first == v1)
        {    vertices[i].second.push_back( {v2, Edge()} ); break;   }
}
...
void GList<Vertex, Edge>::add_non_oriented_non_weighted_edge(const Vertex& v1, const Vertex& v2)
{
    ...
    for(size_t i = 0; i < vertices.size(); i++)
    {
        if(vertices[i].first == v1) {   vertices[i].second.push_back( {v2, Edge() }); }
        else if(vertices[i].first == v2) {  vertices[i].second.push_back( {v1, Edge()}); }
    }
}
...
void GList<Vertex, Edge>::remove_vertex(const Vertex& vertex)
{
    ...
    for (; it != vertices.end(); it++)
        if(it->first == vertex) vertices.erase(it);
}
        ...
        for (size_t i = 0; i < vertices.size(); i++)
        {
            ...
            if(vertices[i].first == v1)
                for(; it != vertices[i].second.end(); it++)
                    if(it->first == v2) {   vertices[i].second.erase(it); break;    }
        }
    }
    else /* non-oriented graph => remove the edge from v1 to v2 and from v2 to v1 */
    {
        for (size_t i = 0; i < vertices.size(); i++)
        {
            ...
            if(vertices[i].first == v2)
                for(; it != vertices[i].second.end(); it++)
                    if(it->first == v1) {   vertices[i].second.erase(it); break;    }
        }
    }
}
...
{
    for(size_t i = 0; i < vertices.size(); i++)
        if(vertices[i].first == vertex) return true;
...
{
    if(vertices.size() == 0 || !has_vertex(vertex)) return;
